<template>
  <div>
    <section
      ...
      v-if="usuarioLogueado && userData && userData.typeUser == 1"
    >
      <nuxt-link to="/admin/productos" class="btn btn-primary rounded-0"
        >Panel de administración</nuxt-link
      >
    </section>
    ...
    <div class="container-fluid front">
      ...
      <div
        class="cart-btn d-none d-lg-block bg-light shadow"
        :class="
          $route.name != 'productos-slug' ? 'cart-btn--medium' : 'cart-btn--top'
        "
        v-if="
          $route.name != 'mi-cuenta' &&
          $route.name != 'pedidos-id' &&
          $route.name != 'carrito' &&
          $route.name != 'finalizar-compra'
        "
      >
        <div
          class="d-flex alig-items-center py-3 px-4"
          @click="mostrarCarrito()"
        >
          <span
            class="icon d-flex justify-content-center align-items-center pr-3"
          >
        ...
        <div class="text-center py-2 cart-btn__item">
          <a
            href=""
            class="text-white d-inline-block w-100"
      ...
      >
        <cart-modal
          v-if="modalCarrito"
          @closeModal="$store.commit('setModalCarrito', false)"
        ></cart-modal>
      </transition>
...
export default {
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      ...
      let sesion = this.$apolloHelpers.getToken() || null;

      this.$store.commit("setUsuarioLogueado", sesion);

      // Asignar datos de usuario logueado
      if (this.$cookies.get(appConfig.userData)) {
        let data = this.$cookies.get(appConfig.userData);

        this.$store.commit("setDatosUsuarioLogueado", data);
      }
    });
  },
  methods: {
